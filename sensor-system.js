...
function evidence(observer,target,b){const r=Math.max(100,dist(observer,target)),sig=shipSignature(target),strength=sensorStrength(observer),clutter=ownClutter(b,observer),passive=(sig.reactor+sig.engines+sig.weapons)*strength*900000/(r*r*clutter),radar=sig.base*strength*520000/(r*r*Math.sqrt(clutter));return passive+radar}
