...
function sensorStrength(s){if(s.kind==='sensorDrone')return s.sensorStrength||1.6;const bridge=alive(s,'bridge').length?.72:0,arrays=alive(s,'sensor').reduce((n,m)=>n+(m.sensitivity||1)*(m.hp/Math.max(1,m.maxHp||m.hp)),0);return(bridge+arrays)*clamp(s.sensorPowerFraction??1,.15,1)}
