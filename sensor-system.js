...
function weaponEmission(s){if(s.runSilent)return 0;let v=0;for(const m of s.modules||[]){if(m.hp<=0||m.disabled||(m.cooldownLeft||0)<=0)continue;if(m.type==='laser')v+=55;if(m.type==='gun')v+=22;if(m.type==='missile')v+=30}return v}
function thermalEmission(s){const h=s.heatState;if(!h)return 0;return Math.max(0,(h.radiatorMW||0)*2.4+(h.fraction||0)*1.5)}
export function shipSignature(s){const base=hullSignature(s),reactor=reactorEmission(s),engines=engineEmission(s),weapons=weaponEmission(s),thermal=thermalEmission(s);return{base,reactor,engines,weapons,thermal,total:base+reactor+engines+weapons+thermal}}
function sensorStrength(s){if(s.kind==='sensorDrone')return s.sensorStrength||1.6;const bridge=alive(s,'bridge').length?0.72:0,arrays=alive(s,'sensor').reduce((n,m)=>n+(m.sensitivity||1)*(m.hp/Math.max(1,m.maxHp||m.hp)),0);return(bridge+arrays)*clamp(s.sensorPowerFraction??1,.15,1)}
function ownClutter(b,s){if(s.kind==='sensorDrone')return 1.08;const own=shipSignature(s),nearOrdnance=(b.ordnance||[]).filter(o=>dist(o,s)<2600).length,nearbyShips=b.ships.filter(o=>o!==s&&!o.dead&&dist(o,s)<3500).reduce((n,o)=>n+Math.min(12,shipSignature(o).total*.08),0);return 1+own.reactor*.045+own.engines*.09+own.weapons*.035+own.thermal*.02+nearOrdnance*.07+nearbyShips*.025}
function evidence(observer,target,b){const r=Math.max(100,dist(observer,target)),sig=shipSignature(target),strength=sensorStrength(observer),clutter=ownClutter(b,observer),passive=(sig.reactor+sig.engines+sig.weapons+sig.thermal)*strength*900000/(r*r*clutter),radar=sig.base*strength*520000/(r*r*Math.sqrt(clutter));return passive+radar}
...
if(typeof window!=='undefined'){window.__sensorGetContact=getContact;window.__sensorKnownEnemies=knownEnemies;window.__sensorSummary=sensorSummary;window.__shipSignature=shipSignature}
