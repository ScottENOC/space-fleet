...
function ewEmission(s){return s.ewJammingActive?(8+(s.ewJamStrength||0)*16):0}
export function shipSignature(s){const base=hullSignature(s),reactor=reactorEmission(s),engines=engineEmission(s),weapons=weaponEmission(s),thermal=thermalEmission(s),ew=ewEmission(s);return{base,reactor,engines,weapons,thermal,ew,total:base+reactor+engines+weapons+thermal+ew}}
...
function ownClutter(b,s){if(s.kind==='sensorDrone')return 1.08;const own=shipSignature(s),nearOrdnance=(b.ordnance||[]).filter(o=>dist(o,s)<2600).length,nearbyShips=b.ships.filter(o=>o!==s&&!o.dead&&dist(o,s)<3500).reduce((n,o)=>n+Math.min(12,shipSignature(o).total*.08),0);return 1+own.reactor*.045+own.engines*.09+own.weapons*.035+own.thermal*.02+own.ew*.025+nearOrdnance*.07+nearbyShips*.025}
function hostileJamming(observer,target,b){let jam=0;for(const s of b.ships){if(s.dead||s.team!==target.team||!s.ewJammingActive)continue;const r=dist(observer,s),range=Math.max(1,s.ewJamRange||0);if(r>range)continue;jam+=(s.ewJamStrength||0)*(1-r/range*.72)}return jam}
