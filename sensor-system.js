...
function sensorStrength(s){const bridge=alive(s,'bridge').length?0.72:0;const arrays=alive(s,'sensor').reduce((n,m)=>n+(m.sensitivity||1)*(m.hp/Math.max(1,m.maxHp||m.hp)),0);return (bridge+arrays)*clamp(s.sensorPowerFraction??1,.15,1);}
...
Battle.prototype.applySystems=function(s,dt){const out=BASE_APPLY.call(this,s,dt);const arrays=alive(s,'sensor'),need=.04e6+arrays.reduce((n,m)=>n+(m.powerUse||0)*(m.hp/Math.max(1,m.maxHp||m.hp)),0);if(s.requestPower&&need>0){const before=s.powerBus?.usedMW||0;s.requestPower(need,'sensors');const supplied=Math.max(0,(s.powerBus?.usedMW||0)-before);s.sensorPowerFraction=clamp(supplied/need,.15,1);if(s.powerState){s.powerState.demandMW=s.powerBus.usedMW;s.powerState.requestedMW=s.powerBus.requestedMW;s.powerState.unmetMW=s.powerBus.unmetMW;s.powerState.groups=s.powerBus.groups;}}else s.sensorPowerFraction=1;return out;};
