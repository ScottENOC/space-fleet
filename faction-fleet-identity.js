...
export const FACTION_FLEET_IDENTITIES={
 ...
 meridian:{label:'Meridian Security',hulls:['frigate_dart','destroyer_guardian','cruiser_pathfinder'],doctrine:'broadside',priority:['weapons','engine','bridge','reactor','hull'],style:'Convoy protection: strong point defence, endurance and threat suppression.',specialisations:['havenEscort']},
 orpheus:{label:'Orpheus Security',hulls:['frigate_line','destroyer_torpedo','cruiser_line'],doctrine:'broadside',priority:['engine','weapons','reactor','bridge','armor','hull'],style:'Heavy industrial security ships: tough, blunt and well supplied.',specialisations:['orpheusArmour']},
 ...
 blackWake:{label:'Black Wake raider',hulls:['frigate_sparrow','destroyer_torpedo','cruiser_missile'],doctrine:'pursuit',priority:['engine','weapons','bridge','reactor','hull'],style:'Elusive raiders favouring deception, missiles, boarding and escape over stand-up fights.',boarding:true,precision:true,ew:true,specialisations:['blackWakeBaffles']}
};
...
function hullPool(factionId,plan=null){const id=FACTION_FLEET_IDENTITIES[factionId]||FACTION_FLEET_IDENTITIES.haven,all=id.hulls.filter(h=>HULLS[h]);if(!plan)return all;const eligible=all.filter(h=>hullAllowedByPlan(HULLS[h],plan));return eligible.length?eligible:[all[0]]}
export function makeFactionBlueprint(factionId,index=0,plan=null){const id=FACTION_FLEET_IDENTITIES[factionId]||FACTION_FLEET_IDENTITIES.haven,pool=hullPool(factionId,plan),hull=pool[index%pool.length],bp=tuneBlueprint(makePremade(hull),factionId);bp.name=`${FACTIONS[factionId]?.short||'Unknown'} ${id.label}`;bp.strategicPosture=plan?.posture||'normal';return bp}
function tuneShip(s,factionId,index=0,plan=null){const id=FACTION_FLEET_IDENTITIES[factionId]||FACTION_FLEET_IDENTITIES.haven;s.factionId=factionId;s.factionName=FACTIONS[factionId]?.name||factionId;s.factionStyle=id.style;s.targetPriority=[...id.priority];s.ai=id.doctrine;s.strategicPosture=plan?.posture||'normal';s.strategicPresence=plan?.presence;
...
export function makeFactionEnemyFleet(factionId,count=1,plan=null){const out=[];for(let i=0;i<count;i++){const bp=makeFactionBlueprint(factionId,i,plan),s=blueprintToShip(bp,'E');out.push(tuneShip(s,factionId,i,plan))}return out}
