...
export function localPowers(system=campaign.location){ensureFactionState();return(SYSTEM_POWERS[system]||[]).map(x=>({...x,presence:campaign.factionPresence?.[system]?.[x.id]??x.presence,faction:FACTIONS[x.id],state:campaign.factions[x.id],playerStanding:campaign.factionRep[x.id]||0})).filter(x=>x.presence>.015).sort((a,b)=>b.presence-a.presence)}
...
export function issuerFor(kind,system=campaign.location){const powers=localPowers(system),meaningful=powers.filter(x=>x.presence>.045),lawful=meaningful.filter(x=>x.faction.lawful),underworld=meaningful.filter(x=>!x.faction.lawful);if(kind==='smuggling')return(underworld.sort((a,b)=>b.presence-a.presence)[0]||powers[0])?.id||'blackWake';if(kind==='antiPiracy')return(lawful.filter(x=>['government','militia','merchant'].includes(x.faction.kind))[0]||lawful[0]||powers[0])?.id||'central';if(kind==='mercenary')return(lawful.filter(x=>x.faction.kind==='government')[0]||lawful[0]||powers[0])?.id||'central';if(kind==='escort'||kind==='trade')return(lawful.filter(x=>x.faction.kind==='merchant')[0]||lawful[0]||powers[0])?.id||'central';if(kind==='scout')return(lawful.filter(x=>['government','industrial','militia'].includes(x.faction.kind))[0]||lawful[0]||powers[0])?.id||'central';if(kind==='government')return'central';return lawful[0]?.id||powers[0]?.id||'central'}
