...
function initialRelation(a,b){if(a===b)return 100;return INITIAL_RELATIONS[pairKey(a,b)]??0}
...
export function ensureFactionState(){
 campaign.factions??={};campaign.factionRelations??={};campaign.factionRep??={};
 for(const id of Object.keys(FACTIONS)){
   const base=DEFAULT_POWER[id]||{power:20,wealth:20,military:20};
   campaign.factions[id]??={...base,status:'active'};
   campaign.factionRep[id]??=defaultPlayerRep(id);
 }
 const ids=Object.keys(FACTIONS);for(let i=0;i<ids.length;i++)for(let j=i+1;j<ids.length;j++){const key=pairKey(ids[i],ids[j]);campaign.factionRelations[key]??=initialRelation(ids[i],ids[j])}
 // Keep the older central standing value in sync so existing legal systems remain compatible.
 campaign.central??={standing:20,lawfulness:0,militaryPermit:false,auxiliary:false,violations:0};
 if(Number.isFinite(campaign.central.standing))campaign.factionRep.central=campaign.central.standing;else campaign.central.standing=campaign.factionRep.central||0;
 saveCampaign(campaign);return campaign.factions;
}
...
export function setRelation(a,b,value){ensureFactionState();if(a===b)return 100;campaign.factionRelations[pairKey(a,b)]=clamp(Math.round(value),-100,100);saveCampaign(campaign);return relation(a,b)}
...
export function factionSummary(id){ensureFactionState();const faction=FACTIONS[id],state=campaign.factions[id];if(!faction||!state)return null;const relations=Object.keys(FACTIONS).filter(x=>x!==id).map(other=>({id:other,value:relation(id,other),label:relationLabel(relation(id,other))})).sort((a,b)=>a.value-b.value);return{id,faction,state,playerStanding:playerStanding(id),best:relations.at(-1),worst:relations[0]}}
