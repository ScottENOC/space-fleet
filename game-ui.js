...
function silhouette(s){
 const cells=s.grid?.validCells||[];if(!cells.length)return `<svg class="fleetSilhouette" viewBox="0 0 40 40"><path d="M20 3L35 33L20 27L5 33Z" fill="rgba(125,184,220,.65)"/></svg>`;
 ...
 return `<svg class="fleetSilhouette" viewBox="0 0 40 40" aria-hidden="true"><g fill="rgba(132,193,226,.72)">${rects}</g></svg>`;
}
...
function installBattleHud(){
 const hud=document.createElement('div');hud.className='battleHud';hud.innerHTML=`
  ...
  <section id="hudShipInfo" class="hudPanel"></section><section id="hudTraffic" class="hudPanel"></section><section id="hudFleetStrip" class="hudPanel"></section>`;
 ...
 // Re-home existing functional controls rather than duplicating their event handlers.
...
function weaponStatus(s){
 const ws=(s.modules||[]).filter(m=>['gun','laser','missile','fighterBay'].includes(m.type)&&m.hp>0&&!m.disabled);if(!ws.length)return'No operational weapons';
 if(s.weaponStatus?.label)return s.weaponStatus.label;
 if(ws.every(w=>Number.isFinite(w.ammo)&&w.ammo<=0))return'Ammunition depleted';
 return s.order||'Executing captain doctrine';
}
function renderFocused(b){const s=focusedShip(b),host=$('#hudShipInfo');if(!s||!host)return;const hp=healthFraction(s),sh=shieldFraction(s),p=s.powerState||{},alive=(s.modules||[]).filter(m=>m.hp>0).length,total=(s.modules||[]).length,stored=p.maxMWh>0?`${Math.round((p.storageFraction||0)*100)}%`:'—';host.innerHTML=`<div class="focusName"><b>${esc(s.name)}</b><span>${s.isFlagship?'FLAGSHIP · ':''}${s.shipClass||''}</span></div><div class="focusBars"><div class="focusBar"><span>Hull</span><div class="barTrack"><div class="barFill ${hp<.35?'healthLow':''}" style="width:${(hp*100).toFixed(1)}%"></div></div><b>${Math.round(hp*100)}%</b></div>${sh>0?`<div class="focusBar"><span>Shield</span><div class="barTrack"><div class="barFill" style="width:${(sh*100).toFixed(1)}%"></div></div><b>${Math.round(sh*100)}%</b></div>`:''}</div><div class="focusGrid"><div><span>Speed</span> ${Math.hypot(s.vx||0,s.vy||0).toFixed(0)} m/s</div><div><span>Turn</span> ${((s.omega||0)*180/Math.PI).toFixed(1)}°/s</div><div><span>Power</span> ${((p.generationMW||0)/1e6).toFixed(1)} MW</div><div><span>Battery</span> ${stored}</div><div><span>Fire</span> ${s.shots||0} / ${s.hits||0} hits</div><div><span>Modules</span> ${alive}/${total}</div><div class="focusStatus">${esc(weaponStatus(s))}</div></div>`;}
function renderFleetStrip(b){const host=$('#hudFleetStrip');if(!host)return;const focus=$('#focusShip')?.value,players=(b?.ships||[]).filter(s=>s.team==='P'),now=performance.now();host.innerHTML=players.map(s=>{const hp=healthFraction(s),f=flashState.get(s.uid)||{},classes=['fleetChip'];if(s.uid===focus)classes.push('focused');if(s.dead)classes.push('dead');if(f.hitUntil>now)classes.push('flashHit');if(f.fireUntil>now)classes.push('flashFire');return `<button class="${classes.join(' ')}" data-focusuid="${s.uid}">${silhouette(s)}<div class="fleetChipText"><b>${esc(s.name)}</b>${s.isFlagship?'★ Admiral':''}</div><div class="fleetMiniBar"><i style="width:${(hp*100).toFixed(1)}%"></i></div></button>`}).join('')+`<button class="fleetChip fleetViewChip" data-allview>ALL<br>SHIPS</button>`;
 host.querySelectorAll('[data-focusuid]').forEach(x=>x.onclick=()=>{const sel=$('#focusShip');if(!sel)return;sel.value=x.dataset.focusuid;sel.dispatchEvent(new Event('change',{bubbles:true}))});host.querySelector('[data-allview]')?.addEventListener('click',()=>$('#fleetView')?.click());}
function watchFlashes(b){for(const s of (b?.ships||[]).filter(x=>x.team==='P')){const hp=(s.modules||[]).reduce((n,m)=>n+Math.max(0,m.hp||0),0),shots=s.shots||0,old=flashState.get(s.uid)||{hp,shots};if(hp<old.hp-.01)old.hitUntil=performance.now()+330;if(shots>old.shots)old.fireUntil=performance.now()+220;old.hp=hp;old.shots=shots;flashState.set(s.uid,old)}}
...
function refresh(){const b=window.__fleetBattle;syncNonBattleUI();if($('#battle')?.classList.contains('active')&&b){if(b!==lastBattle){lastBattle=b;resultShownFor=null;flashState.clear()}watchFlashes(b);renderFocused(b);renderFleetStrip(b);showResult(b)}}
