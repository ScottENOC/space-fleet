...
export function makeTradingPremade(hullId='trader_mule'){
 ...
 const drive=h.cellCount>180?'engine_4':'engine_3';firstFit(bp,drive,3);firstFit(bp,drive,3);
 firstFit(bp,'gun_1',0);firstFit(bp,'gun_1',2);
 const cargo=h.cellCount>220?'cargo_3':h.cellCount>120?'cargo_2':'cargo_1';while(firstFit(bp,cargo)){}while(firstFit(bp,'cargo_1')){}
 return bp;
}
export function cargoCapacityFromBlueprint(bp){return (bp.placements||[]).reduce((n,p)=>n+(MODULES[p.moduleId]?.cargoCapacity||0),0)}
export function armamentClass(bp){let guns=0,heavyGuns=0,lasers=0,missiles=0,fighters=0;for(const p of bp.placements||[]){const m=MODULES[p.moduleId];if(!m)continue;if(m.type==='gun'){guns++;if((m.damage||0)>100)heavyGuns++}else if(m.type==='laser')lasers++;else if(m.type==='missile')missiles++;else if(m.type==='fighterBay')fighters+=m.fighters||0;}const h=HULLS[bp.hullId];if(!h?.civilianHull)return{classification:'military',guns,lasers,missiles,fighters,reason:'purpose-built naval hull'};if(lasers||missiles||fighters||heavyGuns||guns>4)return{classification:'military',guns,lasers,missiles,fighters,reason:lasers||missiles?'restricted weapons fit':'armament exceeds civilian defensive allowance'};if(guns>2)return{classification:'armed merchant',guns,lasers,missiles,fighters,reason:'heavy defensive gun fit'};return{classification:'civilian merchant',guns,lasers,missiles,fighters,reason:'ordinary defensive armament'};}
