...
function renderContracts(){const host=document.querySelector('#contractBoard'),contracts=generateContracts();host.innerHTML=contracts.map((c,i)=>`<article class="contract ${c.legal?'legal':'grey'}"><h3>${esc(c.title)} · ${c.pay} cr</h3><p>${esc(c.text)}</p><small>${c.legal?'Recognised legal work':'Off-books / legal risk'}</small><button data-contract="${i}">Accept</button></article>`).join('');host.querySelectorAll('[data-contract]').forEach(b=>b.onclick=()=>{const c=contracts[+b.dataset.contract];if(c.encounter==='meteorSwarm'){campaign.pendingEncounter={kind:'hazardEscort',title:c.title,contract:c,convoyCount:c.convoyCount||2,day:campaign.day};saveCampaign(campaign);launchCampaignBattle();return;}if(c.kind==='antiPiracy'||c.kind==='mercenary'){campaign.pendingEncounter={kind:'fight',title:c.title,contract:c,day:campaign.day};saveCampaign(campaign);launchCampaignBattle();return;}completeAbstractContract(c);currentChoices=choices();render()})}
...
function installBattleOutcomeControls(){
 ...
 const selected=()=>{const b=getBattle();if(!b)return[];const who=document.querySelector('#orderShips')?.value||'all';return b.ships.filter(s=>s.team==='P'&&!s.dead&&!s.civilianEscort&&(who==='all'||s.uid===who))};
 document.querySelector('#withdrawShips').onclick=()=>{const b=getBattle();if(b)orderWithdraw(b,selected())};
 document.querySelector('#withdrawFleet').onclick=()=>{const b=getBattle();if(b)orderWithdraw(b,b.ships.filter(s=>s.team==='P'&&!s.dead&&!s.civilianEscort))};
...
function watchBattle(){const b=getBattle();if(!b||!b.campaignBattle)return;if(b.winner&&!b._campaignPersisted){const encounter=campaign.pendingEncounter,contract=encounter?.contract,hazard=b.hazardResult;persistBattleResults(b);campaign.pendingEncounter=null;campaign.day+=1;if(b.winner==='P'){const factor=hazard?.payoutFactor??1,gain=Math.round((contract?.pay||180)*factor);campaign.credits+=gain;campaign.reputation+=hazard?.survivors===hazard?.total?2:1;if(contract){campaign.contractsCompleted[contract.kind]=(campaign.contractsCompleted[contract.kind]||0)+1;campaign.central.standing+=(contract.legal?2:-2);}if(hazard)campaign.log.push(`Day ${campaign.day}: meteor escort complete — ${hazard.survivors}/${hazard.total} merchants survived; ${hazard.destroyed} meteors destroyed; ${gain} cr paid.`);else campaign.log.push(`Day ${campaign.day}: fleet action won; ${gain} cr recovered/paid.`)}else if(hazard)campaign.log.push(`Day ${campaign.day}: meteor escort failed; convoy lost in the hazard corridor.`);else campaign.log.push(`Day ${campaign.day}: fleet action ended without victory.`);saveCampaign(campaign);render()}}
