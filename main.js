...
import {HULLS,MODULES,blueprintToShip,designStats,footprint,canPlace,placeModule,removeAt,presetBroadside,presetPursuit,presetForHull} from './shipyard.js';
...
let designs={A:presetBroadside(),B:presetPursuit()},activeSlot='A',selected='gun_1',rotation=0,erase=false;
...
const moduleColours={gun:'#dedede',laser:'#ff7c7c',missile:'#d88962',fighterBay:'#8fc87b',armor:'#777',reactor:'#f4cf63',shield:'#79e0ff',engine:'#56b6ff',bridge:'#d9e1ec',radiator:'#a895c7'};
const moduleLabels={gun:'G',laser:'L',missile:'M',fighterBay:'F',armor:'A',reactor:'R',shield:'S',engine:'E',bridge:'B',radiator:'D'};
...
function initShipyard(){
  $('#hull').innerHTML=Object.values(HULLS).map(h=>`<option value="${h.id}">${h.shipClass} · ${h.name} · ${h.cellCount} cells</option>`).join('');
...
function bindYard(){
  ...
  $('#presetHull').onclick=()=>{designs[activeSlot]=presetForHull($('#hull').value);syncYard()};
...
function renderGrid(){
  ...
  const st=designStats(b),side=activeSlot==='A'?'Player':'Enemy';$('#designStats').innerHTML=`<b>${side}: ${b.name}</b> · ${h.shipClass} · ${h.cellCount} hull cells<br>${st.cellsUsed}/${st.cellsTotal} occupied · ${st.mass.toFixed(0)} t · ${(st.power/1e6).toFixed(0)} MW generation · ${(st.maxPowerUse/1e6).toFixed(1)} MW theoretical load · ${(st.thrust/1e6).toFixed(2)} MN thrust · ${st.weapons} ship weapons${st.fighters?` · ${st.fighters} fighters`:''}`;
}
...
function syncTargetMenu(){if(!battle)return;const recipients=$('#orderShips'),targets=$('#orderTarget');recipients.innerHTML='<option value="all">All player ships</option>'+battle.ships.filter(s=>s.team==='P').map(s=>`<option value="${s.uid}">${s.name}</option>`).join('');targets.innerHTML='<option value="any">Any enemy ship</option>'+battle.ships.filter(s=>s.team==='E').map(s=>`<option value="${s.uid}">${s.name}</option>`).join('')}
$('#applyTargetOrder').onclick=()=>{const ships=applyTargetOrder(battle,{recipient:$('#orderShips').value,target:$('#orderTarget').value,priority:$('#orderPriority').value});const target=$('#orderTarget').selectedOptions[0]?.textContent||'any enemy';const priority=$('#orderPriority').selectedOptions[0]?.textContent||'default';battle.log(`Player order: ${ships.map(s=>s.name).join(', ')} target ${target}; ${priority}.`)};

function updateCamera(){
  const w=canvas.clientWidth,h=canvas.clientHeight;if(!w||!h)return;
  if(cameraMode==='focus'){const s=focusShip();if(!s)return;camera.x=s.x;camera.y=s.y;camera.scale=Math.max(.02,Math.min((w*.20)/Math.max(s.length,1),(h*.20)/Math.max(s.width,1)));$('#cameraReadout').textContent=`Focus · ${s.name} · ship ${(s.length*camera.scale).toFixed(0)}×${(s.width*camera.scale).toFixed(0)} px`;}
...
function drawOrdnance(){for(const o of battle.ordnance||[]){const [x,y]=worldToScreen(o.x,o.y);if(x<0||x>canvas.clientWidth||y<0||y>canvas.clientHeight)continue;ctx.save();ctx.translate(x,y);ctx.rotate(o.angle||0);ctx.fillStyle=o.kind==='fighter'?'#9fd18c':'#ff9b61';ctx.beginPath();if(o.kind==='fighter'){ctx.moveTo(5,0);ctx.lineTo(-4,-3);ctx.lineTo(-2,0);ctx.lineTo(-4,3)}else{ctx.moveTo(5,0);ctx.lineTo(-4,-2.5);ctx.lineTo(-4,2.5)}ctx.closePath();ctx.fill();ctx.restore()}}
function drawFocusTelemetry(){if(cameraMode!=='focus')return;const s=focusShip();if(!s)return;const active=s.modules.filter(m=>m.type==='engine'&&m.hp>0&&m.active);ctx.fillStyle='rgba(8,13,20,.78)';ctx.fillRect(10,10,235,58);ctx.fillStyle='#e8edf4';ctx.font='11px system-ui';ctx.fillText(`ω ${(s.omega*180/Math.PI).toFixed(1)}°/s · heading ${(s.angle*180/Math.PI).toFixed(0)}°`,18,29);ctx.fillText(`engines firing ${active.length} · speed ${Math.hypot(s.vx,s.vy).toFixed(0)} m/s`,18,45);ctx.fillStyle='#9cb0c9';ctx.fillText(s.order||'',18,61)}
