...
export const HULLS={
 escort:{id:'escort',name:'Needle-class frigate frame',role:'Fast escort',rows:['....##....','...####...','..######..','.########.','##########','##########','.########.','..######..','...####...'],frameMass:62,frameHp:850,ai:'pursuit'},
 line:{id:'line',name:'Bastion-class frigate frame',role:'Line / broadside frigate',rows:['...######...','..########..','.##########.','############','############','############','############','.##########.','..########..','...######...'],frameMass:105,frameHp:1350,ai:'broadside'},
 raider:{id:'raider',name:'Corsair-class frigate frame',role:'Asymmetric raider',rows:['.....##....','....####...','..########.','###########','###########','..########.','....####...','.....##....'],frameMass:72,frameHp:950,ai:'pursuit'}
};
...
export function presetBroadside(){const b=emptyBlueprint('line','Resolute-class broadside frigate','broadside'),add=(id,x,y,r=0)=>placeModule(b,id,x,y,r);add('reactor',5,4);add('bridge',8,4);add('shield',3,4);add('engine',0,3,0);add('engine',0,5,0);add('thruster',9,2,2);add('thruster',9,7,2);add('armor',3,2,0);add('armor',5,2,0);add('armor',7,2,0);add('armor',3,7,0);add('armor',5,7,0);add('armor',7,7,0);add('gun',3,0,3);add('gun',5,0,3);add('gun',7,0,3);add('gun',3,8,1);add('gun',5,8,1);add('gun',7,8,1);return b}
export function presetPursuit(){const b=emptyBlueprint('escort','Vigilant-class pursuit frigate','pursuit'),add=(id,x,y,r=0)=>placeModule(b,id,x,y,r);add('reactor',4,4);add('bridge',6,4);add('shield',6,2);add('armor',7,4,1);add('engine',1,3,0);add('engine',1,5,0);add('thruster',4,1,1);add('thruster',5,7,3);add('thruster',7,3,2);add('thruster',7,5,2);add('gun',5,1,0);add('gun',5,6,0);add('laser',8,4,0);return b}
