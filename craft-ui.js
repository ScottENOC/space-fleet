...
function craftCounts(s,b){
 ...
 return{inv,air,fighters,drones,returning,returnF,returnD};
}
...
function bayRows(s,b){return bays(s).map((bay,i)=>{const slots=bay._slotUntil||Array(bay.launchSlots||1).fill(0),busy=slots.filter(x=>x>b.t),soon=busy.length?Math.max(0,Math.min(...busy)-b.t):0,util=slots.length?busy.length/slots.length:0;return `<div class="flightRow bayRow"><span>Bay ${i+1}</span><b>${busy.length}/${slots.length} slots busy</b><small>${busy.length?`next clear ${soon.toFixed(1)}s`:'ready'} · ${pct(util)} occupied</small><div class="flightMiniTrack"><i style="width:${pct(util)}"></i></div></div>`}).join('')}
function renderFocusedFlightOps(){
 ...
 const c=craftCounts(s,b),initialF=hangars(s).reduce((n,h)=>n+(h.initialCraft?.fighter??h.defaultCraft?.fighter??0),0),initialD=hangars(s).reduce((n,h)=>n+(h.initialCraft?.sensorDrone??h.defaultCraft?.sensorDrone??0),0),knownF=c.inv.fighter+c.fighters.length+(c.inv.servicingFighter||0),knownD=c.inv.sensorDrone+c.drones.length+(c.inv.servicingDrone||0),lostF=Math.max(0,initialF-knownF),lostD=Math.max(0,initialD-knownD);
 const wrap=document.createElement('section');wrap.className='focusedFlightOps';wrap.innerHTML=`<div class="flightFocusHead"><b>FLIGHT OPERATIONS</b><span>${reserveLabel(s.fighterReserveFraction??.25)}</span></div><div class="flightSummary"><div><span>Fighters</span><b>${c.inv.fighter} ready · ${c.fighters.length} airborne</b><small>${c.returnF} returning${lostF?` · ${lostF} lost`:''}</small></div><div><span>Drones</span><b>${c.inv.sensorDrone} ready · ${c.drones.length} airborne</b><small>${c.returnD} returning${lostD?` · ${lostD} lost`:''}</small></div></div>${hangarRows(s,b)}${bayRows(s,b)}${c.returning.length?`<div class="recoveryQueue"><b>Recovery queue</b>${c.returning.slice(0,5).map(o=>`<span>${o.kind==='fighter'?'F':'D'} · ${o.name}${o.recoveryShip&&o.recoveryShip!==o.homeShip?` → divert ${o.recoveryShip.name}`:''} · fuel ${Math.round((o.fuel/o.maxFuel)*100)}%</span>`).join('')}${c.returning.length>5?`<span>+${c.returning.length-5} more</span>`:''}</div>`:''}`;host.append(wrap);
}
function refresh(){
