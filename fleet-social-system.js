...
import {ensureHumanCrew} from './crew-human-factors.js';
...
function maybeComment(b,s){
 ...
 b.crewInbox??=[];b.crewInbox.push({id:`social:${s.uid}:${Math.floor(b.t)}`,t:b.t,shipUid:s.uid,shipName:s.name,role:'captain',officer:c.name,title:'Captain',severity:2,text,request:null,status:'info'});
}
...
export function captainOpinions(s,b){return b.ships.filter(o=>o.team===s.team&&o!==s).map(o=>({ship:o,opinion:rel(s,o)}));}
