...
function enforce(){let dirty=false;for(const f of campaign.strategicFronts?.fleets||[]){if(!['deployed','enroute'].includes(f.status))continue;const system=f.status==='enroute'?f.destination:f.system,front=activeFront(f.frontId),mission=f.mission||'patrol';if(strategicMissionAllowed(f.factionId,system,mission,front)){if(['raid','blockade'].includes(mission))dirty=recordAccessIncursion(f.factionId,system,mission)||dirty;continue}if(['patrol','convoyEscort','reinforce'].includes(mission))dirty=sendHome(f)||dirty;else dirty=recordAccessIncursion(f.factionId,system,mission)||dirty}if(dirty)saveCampaign(campaign);return dirty}
