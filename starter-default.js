import './crew-escalation.js?v=61';
import './mobile-ui.js?v=66';
import './battle-context-ui.js?v=65';
import './pdc-system.js?v=74';
import './mystery-ui.js?v=73';
import './mystery-encounter-hooks.js?v=73';
import './expedition-battle-hooks.js?v=93';
import './tactical-hazards.js?v=74';
import './tactical-hazard-ui.js?v=74';
import './interdiction-system.js?v=75';
import './interdiction-ui.js?v=75';
import './interdiction-campaign-hooks.js?v=75';
import './tactical-missions.js?v=76';
import './tactical-mission-ui.js?v=76';
import './faction-strategy.js?v=98';
import './strategic-fronts.js?v=99';
import './strategic-fronts-ui.js?v=103';
import './strategic-mission-events.js?v=104';
import './strategic-mission-events-ui.js?v=104';
import './faction-goals.js?v=105';
import './faction-politics.js?v=106';
import './faction-treaties.js?v=107';
import './treaty-operations.js?v=108';
import './treaty-breach-recovery.js?v=108';
import './treaty-fleet-enforcement.js?v=108';
import './player-security-invocations.js?v=108';
import './treaty-crisis-ai.js?v=108';
import './strategic-economy.js?v=107';
import './strategic-logistics.js?v=104';
import './strategic-map.js?v=104';
import './strategic-map-ui.js?v=104';
import './shipyard-capacity.js?v=107';
import './shipyard-capacity-ui.js?v=107';
import './strategic-logistics-ui.js?v=104';
import './strategic-economy-ui.js?v=107';
import './faction-goals-ui.js?v=105';
import './faction-politics-ui.js?v=106';
import './faction-treaties-ui.js?v=107';
import './treaty-operations-ui.js?v=108';
import './combat-objectives.js?v=78';
import './combat-objective-ui.js?v=78';
import './main-plot.js?v=80';
import './lacuna-transit.js?v=81';
import './lacuna-transit-guard.js?v=82';
import './lacuna-transit-ui.js?v=81';
import './lacuna-survival.js?v=82';
import './lacuna-survival-guard.js?v=82';
import './main-plot-ui.js?v=81';
import './lacuna-survival-ui.js?v=82';
import './lacuna-activity.js?v=83';
import './lacuna-activity-ui.js?v=83';
import './lacuna-inner-system.js?v=84';
import './lacuna-inner-system-ui.js?v=84';
import './lacuna-concord.js?v=85';
import './lacuna-concord-ui.js?v=85';
import './lacuna-vault.js?v=86';
import './lacuna-vault-ui.js?v=86';
import './lacuna-orison.js?v=87';
import './lacuna-orison-ui.js?v=87';
import './lacuna-orison-core.js?v=88';
import './lacuna-orison-core-ui.js?v=88';
import './peregrine-transit.js?v=89';
import './peregrine-transit-ui.js?v=89';
import './peregrine-contact.js?v=90';
import './peregrine-contact-ui.js?v=90';
import './peregrine-council.js?v=91';
import './peregrine-council-ui.js?v=91';
import './peregrine-return-engineering.js?v=92';
import './peregrine-return-engineering-ui.js?v=92';
import './peregrine-return-decision.js?v=93';
import './peregrine-return-decision-ui.js?v=93';
import './campaign-intelligence.js?v=94';
import './lacuna-disclosure.js?v=94';
import './lacuna-disclosure-ui.js?v=94';
import './contact-protocol.js?v=95';
import './lacuna-first-arrival.js?v=95';
import './lacuna-first-arrival-ui.js?v=95';
import './lacuna-corridor.js?v=97';
import './lacuna-corridor-ui.js?v=97';
import './corridor-traffic.js?v=98';
import './corridor-traffic-ui.js?v=97';
if(!document.querySelector('link[href*="mystery-ui.css"]')){const link=document.createElement('link');link.rel='stylesheet';link.href='mystery-ui.css?v=73';document.head.append(link)}
document.querySelector('#presetPursuit')?.click();