...
const MISSILE_CLEARANCE_TIME=.38;
const MISSILE_LAUNCH_SPEED=170;
const FRIENDLY_CORRIDOR=90;
...
function segmentDistance(px,py,ax,ay,bx,by){const abx=bx-ax,aby=by-ay,den=abx*abx+aby*aby;if(den<1e-6)return Math.hypot(px-ax,py-ay);const u=clamp(((px-ax)*abx+(py-ay)*aby)/den,0,1),x=ax+abx*u,y=ay+aby*u;return Math.hypot(px-x,py-y)}
function missileLaunchSafe(b,s,w,t){const p=worldPoint(s,w),a=s.angle+(w.mountDir||0),accel=Math.max(120,(w.missileThrust||85000)/320),clearance=MISSSILE_CLEARANCE_DISTANCE(accel),cx=p.x+Math.cos(a)*clearance,cy=p.y+Math.sin(a)*clearance;for(const f of b.ships){if(f===s||f===t||f.dead||f.team!==s.team)continue;const r=(f.radius||15)+FRIENDLY_CORRIDOR;if(segmentDistance(f.x,f.y,p.x,p.y,cx,cy)<r)return false;const tx=t.x,ty=t.y;if(segmentDistance(f.x,f.y,cx,cy,tx,ty)<r&&dist(p,f)<dist(p,t)*1.1)return false}return true}
function MISSSILE_CLEARANCE_DISTANCE(accel){return MISSILE_LAUNCH_SPEED*MISSILE_CLEARANCE_TIME+.5*accel*MISSILE_CLEARANCE_TIME*MISSILE_CLEARANCE_TIME}
function missileDeltaVFeasible(s,w,t){const accel=Math.max(120,(w.missileThrust||85000)/320),fuel=w.missileFuel||9,dv=accel*fuel,rx=t.x-s.x,ry=t.y-s.y,d=Math.hypot(rx,ry)||1,nx=rx/d,ny=ry/d,rvx=(t.vx||0)-(s.vx||0),rvy=(t.vy||0)-(s.vy||0),receding=Math.max(0,rvx*nx+rvy*ny),crossing=Math.abs(rvx*ny-rvy*nx),geometryCost=receding+crossing*.45;return dv>geometryCost+Math.min(700,d*.08)}
function launchMissile(b,s,w,t){if((w.ammo||0)<=0||!missileDeltaVFeasible(s,w,t)||!missileLaunchSafe(b,s,w,t)||!pay(s,w))return false;const p=worldPoint(s,w),a=wrap(s.angle+(w.mountDir||0)),accel=Math.max(120,(w.missileThrust||85000)/320);b.ordnance??=[];b.ordnance.push({kind:'missile',team:s.team,owner:s,x:p.x,y:p.y,vx:s.vx+Math.cos(a)*MISSILE_LAUNCH_SPEED,vy:s.vy+Math.sin(a)*MISSILE_LAUNCH_SPEED,angle:a,launchAngle:a,launchTime:b.t,guidanceActiveAt:b.t+MISSILE_CLEARANCE_TIME,ownerSafeUntil:b.t+MISSILE_CLEARANCE_TIME+.18,hp:w.missileHp||24,maxHp:w.missileHp||24,damage:w.damage||100,fuel:w.missileFuel||9,accel,turnRate:1.9,target:t,targetOrdnance:null,r:1.3,ttl:Infinity,penetration:profile(w).armorPen,shieldHardness:profile(w).shieldHardness});w.ammo--;w.cooldownLeft=w.cooldown||4;s.shots++;return true}
