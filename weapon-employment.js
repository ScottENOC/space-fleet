...
const PROFILES={
 gun:{
  light:{tracking:1.55,ideal:'small',shieldHardness:0.55,armorPen:0.75},
  medium:{tracking:.95,ideal:'medium',shieldHardness:.85,armorPen:1.05},
  heavy:{tracking:.48,ideal:'large',shieldHardness:1.35,armorPen:1.55}
 },
 laser:{
  light:{tracking:2.15,ideal:'small',shieldHardness:1.20,armorPen:.35},
  medium:{tracking:1.25,ideal:'medium',shieldHardness:1.55,armorPen:.48},
  heavy:{tracking:.62,ideal:'large',shieldHardness:2.05,armorPen:.62}
 },
 missile:{
  light:{tracking:1.45,ideal:'small',shieldHardness:.95,armorPen:1.0},
  medium:{tracking:1.0,ideal:'medium',shieldHardness:1.25,armorPen:1.2},
  heavy:{tracking:.68,ideal:'large',shieldHardness:1.75,armorPen:1.45}
 }
};
...
function targetValue(b,s,w,t,main){if(t.dead||t.team===s.team||!inArc(s,w,t)||!inRange(s,w,t))return-Infinity;const q=sensorQuality(b,s,t);if(q<.08)return-Infinity;const p=profile(w),scale=targetScale(t),ang=angularRate(s,t),trackScore=clamp(1-ang/Math.max(.05,p.tracking),0,1),sh=shieldState(t),arm=armourFactor(t);let score=1;
 if(t===main)score+=2.6;
 if(scale===p.ideal)score+=.65;else if((scale==='small'&&p.ideal==='large')||(scale==='large'&&p.ideal==='small'))score-=.9;
 score+=trackScore*1.4-(1-trackScore)*1.8;
 score+=q*.45;
 const d=dist(s,t);score+=clamp(1-d/Math.max(1,w.range||12000),0,.45);
 // Avoid spending weak attacks on targets whose defences will erase them.
 if(sh.charge>0){const effective=(w.damage||0)*p.shieldHardness;const perHitFloor=sh.hard||Math.max(0,sh.regen*.10);if(effective<=perHitFloor*1.05)score-=4.5;else if(effective<perHitFloor*1.7)score-=1.4;if(w.type==='laser')score+=.7;}
 if(arm>.70&&p.armorPen<.8)score-=2.6;
 if(w.type==='missile'){if((w.ammo||0)<=2)score-=.9;if(scale==='small'&&sizeClass(w)==='heavy')score-=1.1;}
 // Nearby vulnerable targets invite opportunistic fire even if not the main effort.
 if(t!==main&&d<1800&&sh.charge<=0)score+=1.35;
 return score;}
...
function chooseModule(s,t){const alive=(t.modules||[]).filter(m=>m.hp>0&&!m.disabled);for(const p of s.targetPriority||[]){const type=p==='weapons'?['gun','laser','missile']: [p];const m=alive.find(x=>type.includes(x.type));if(m)return m}return alive.find(m=>m.type!=='hull')||alive[0]||null}
...
Battle.prototype.fireWeapons=function(s,e,powerBudget){const main=mainTarget(this,s);if(!main)return;const report=[];for(const w of (s.modules||[]).filter(m=>['gun','laser','missile'].includes(m.type)&&m.hp>0&&!m.disabled&&(m.cooldownLeft||0)<=0)){const t=chooseTarget(this,s,w,main);if(!t)continue;let fired=false;if(w.type==='gun')fired=fireGun(this,s,w,t);else if(w.type==='laser')fired=fireLaser(this,s,w,t);else fired=launchMissile(this,s,w,t);if(fired)report.push({weapon:w.id||w.name,target:t.uid,main:t===main})}s.weaponEmployment=report;};
